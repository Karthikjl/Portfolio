...
const ANIMATION_PRESETS = {
  none:   { duration: 0,    stagger: 0,    distance: 0,   easing: 'ease' },
  subtle: { duration: 0.4,  stagger: 0.06, distance: 15,  easing: 'ease-out' },
  medium: { duration: 0.6,  stagger: 0.1,  distance: 30,  easing: 'cubic-bezier(0.4,0,0.2,1)' },
  heavy:  { duration: 1.0,  stagger: 0.15, distance: 60,  easing: 'cubic-bezier(0.16,1,0.3,1)' },
}

// ── SCROLL REVEAL HOOK ────────────────────────────────
function useScrollReveal(preset = 'medium') {
  const ref = useRef(null)
  const [visible, setVisible] = useState(false)
  const cfg = ANIMATION_PRESETS[preset] || ANIMATION_PRESETS.medium

  useEffect(() => {
    if (cfg.duration === 0) { setVisible(true); return }
  ...
  const style = {
    opacity: visible ? 1 : 0,
    transform: visible ? 'translateY(0)' : `translateY(${cfg.distance}px)`,
    transition: `opacity ${cfg.duration}s ${cfg.easing}, transform ${cfg.duration}s ${cfg.easing}`,
  }
  return { ref, style, visible, cfg }
}
...
function GlitchText({ text, style }) {
    ...
    const schedule = () => {
      ...
      setTimeout(() => {
        ...
        const id = setInterval(() => {
          ...
          if (++frame >= 12) { clearInterval(id); setDisplay(text); setGlitching(false) }
        }, 55)
        schedule()
      }, delay)
    }
  ...
  return (
    <div style={{ ...style, position: 'relative', filter: glitching ? 'hue-rotate(80deg)' : 'none', transition: 'filter 0.05s' }}>
      ...
      {glitching && (
        <span aria-hidden="true" style={{ position: 'absolute', top: 0, left: 0, whiteSpace: 'nowrap', overflow: 'hidden', width: '100%', color: 'inherit' }}>
          {display}
        </span>
      )}
      {glitching && (
...
// ── SECTION HEADER ────────────────────────────────────
function SectionHeader({ label, sub, animPreset }) {
  const { ref, style } = useScrollReveal(animPreset)
  return (
    <div ref={ref} style={{ ...style, display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
...
function RevealCard({ index, preset, children, noStretch = false }) {
  const cfg = ANIMATION_PRESETS[preset] || ANIMATION_PRESETS.medium
  const ref = useRef(null)
  const [visible, setVisible] = useState(false)
  useEffect(() => {
  ...
  return (
    <div ref={ref} style={{
      opacity: visible ? 1 : 0,
      transform: visible ? 'translateY(0) scale(1)' : `translateY(${cfg.distance}px) scale(0.97)`,
      transition: `opacity ${cfg.duration}s ${cfg.easing} ${index * cfg.stagger}s, transform ${cfg.duration}s ${cfg.easing} ${index * cfg.stagger}s`,
      alignSelf: noStretch ? 'flex-start' : 'stretch',
      display: 'flex', flex: '1 1 300px', minWidth: '280px',
...
export default function Portfolio() {
  ...
  const animPreset = settings['anim_preset'] || 'medium'
  ...
  // Hero scroll reveal styles
  const heroDelay = (n) => ({
    opacity: booted ? 1 : 0,
    transform: booted ? 'translateY(0)' : 'translateY(20px)',
    transition: `opacity 0.6s ease ${n * 0.15}s, transform 0.6s ease ${n * 0.15}s`,
  })
  ...
  return (
    <div className="no-cursor" style={{ position: 'relative', minHeight: '100vh' }}>
      ...
      {/* BG TOGGLE */}
      <button onClick={toggleMatrix} title={matrixOn ? 'Turn off background' : 'Turn on background'} style={{
        ...
        padding: '0.45rem 0.9rem', cursor: 'pointer', transition: 'all 0.2s ease',
        boxShadow: matrixOn ? '0 0 12px var(--green-glow)' : 'none',
        display: 'flex', alignItems: 'center', gap: '0.5rem',
      }}>
        <span style={{ display: 'inline-block', width: 7, height: 7, borderRadius: '50%', background: matrixOn ? 'var(--green)' : 'var(--muted)', boxShadow: matrixOn ? '0 0 6px var(--green)' : 'none', transition: 'all 0.2s' }} />
        {matrixOn ? 'BG ON' : 'BG OFF'}
      </button>

      {/* TOPBAR */}
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, background: 'rgba(10,10,10,0.97)', borderBottom: '1px solid var(--border)', backdropFilter: 'blur(12px)', height: '56px' }}>
      ...
      <div className={`mobile-nav${menuOpen ? ' open' : ''}`}>
        {NAV_ITEMS.map(item => (
          <button key={item} onClick={() => scrollTo(item)} style={{
            background: 'none', border: 'none', fontFamily: 'var(--font-orb)', fontSize: '1.8rem',
            color: activeSection === item ? 'var(--green)' : 'var(--muted)',
        ...
        <div style={{ fontSize: '0.6rem', color: 'var(--muted)', letterSpacing: '0.12em', marginTop: '1rem' }}>UP {fmt(uptime)}</div>
      </div>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 1200, margin: '0 auto', padding: '88px 2rem 5rem' }}>
        ...
        <section style={{ minHeight: '90vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingBottom: '2rem' }}>
          <div style={{ ...heroDelay(0), fontSize: '0.72rem', color: 'var(--muted)', letterSpacing: '0.15em', marginBottom: '1.2rem' }}>
          ...
          <div style={heroDelay(1)}>
            <GlitchText text={ME.name} style={{
              fontFamily: 'var(--font-vt)', fontSize: 'clamp(4rem, 12vw, 9rem)',
              lineHeight: 0.9, color: 'var(--green)',
              textShadow: '0 0 20px var(--green), 0 0 60px var(--green-dim)',
              letterSpacing: '0.04em', marginBottom: '0.7rem',
            }} />
          </div>

          <div style={{ ...heroDelay(2), fontFamily: 'var(--font-orb)', fontSize: 'clamp(0.65rem, 2vw, 0.95rem)', color: 'var(--amber)', letterSpacing: '0.35em', textShadow: '0 0 12px var(--amber)', marginBottom: '2.5rem' }}>
            // {ME.role}
          </div>

          <div style={{ ...heroDelay(3), maxWidth: 580, fontSize: '0.83rem', color: 'var(--green-dim)', lineHeight: 1.95, borderLeft: '2px solid var(--green-dim)', paddingLeft: '1.2rem', marginBottom: '3rem', minHeight: '5rem' }}>
            <span style={{ color: 'var(--muted)' }}>&gt; </span>
            {bioTyped}
            {!bioDone && <span className="cursor" />}
          </div>

          <div style={{ ...heroDelay(4), display: 'flex', gap: '0.8rem', flexWrap: 'wrap', marginBottom: '4rem' }}>
            ...
            <a href={ME.github} target="_blank" rel="noopener noreferrer" className="btn btn-cyan" style={{ padding: '0.75rem 2rem', fontSize: '0.8rem', textDecoration: 'none' }}>
              <span>⌥ GITHUB</span>
            </a>
          </div>

          <div style={{ ...heroDelay(5), display: 'flex', flexWrap: 'wrap', border: '1px solid var(--border)', background: 'var(--black)' }}>
            {[
              ...
              <div key={s.label} style={{ borderRight: i < 3 ? '1px solid var(--border)' : 'none', padding: '1.2rem 2rem', flex: '1 0 120px' }}>
                <div style={{ fontFamily: s.small ? 'var(--font-mono)' : 'var(--font-vt)', fontSize: s.small ? '1.1rem' : '2.8rem', color: s.color, lineHeight: 1, textShadow: `0 0 10px ${s.color}`, marginBottom: '0.5rem' }}>{s.value}</div>
        ...
        <section id="PROJECTS" style={{ paddingBottom: '6rem', scrollMarginTop: '60px' }}>
          <SectionHeader label="PROJECTS" sub={`${filtered.length} RECORDS`} animPreset={animPreset} />
          ...
          {loading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(360px, 100%), 1fr))', gap: '1rem' }}>
              {filtered.map((project, i) => (
                <RevealCard key={project.id} index={i} preset={animPreset}>
        ...
        <section id="ABOUT" style={{ paddingBottom: '6rem', scrollMarginTop: '60px' }}>
          <SectionHeader label="ABOUT" sub="SYSTEM INFO" animPreset={animPreset} />
          <div style={{ display: 'flex', flexWrap: 'wrap', border: '1px solid var(--border)', background: 'var(--black)' }}>

            {/* Identity JSON */}
            <RevealCard index={0} preset={animPreset} noStretch>
              <div style={{ background: 'var(--black)', padding: '1.2rem', width: '100%', borderRight: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '1rem', paddingBottom: '0.6rem', borderBottom: '1px solid var(--border)' }}>
                  {['#ff2244','#ffb000','#00ff41'].map(c => (
                    <span key={c} style={{ width: 10, height: 10, borderRadius: '50%', background: c, display: 'inline-block', boxShadow: `0 0 4px ${c}` }} />
                  ))}
                  <span style={{ fontSize: '0.62rem', color: 'var(--muted)', marginLeft: '0.5rem', letterSpacing: '0.1em' }}>identity.json</span>
                </div>
                <div style={{ fontSize: '0.78rem', fontFamily: 'var(--font-mono)', lineHeight: 1.8 }}>
                  ...
                  {[
                    ...
                    <div key={k} style={{ paddingLeft: '1rem', margin: '0.3rem 0', display: 'flex', flexWrap: 'wrap', alignItems: 'baseline', columnGap: '0.4rem' }}>
                      <span style={{ color: 'var(--cyan)', whiteSpace: 'nowrap' }}>"{k}"</span>
                      <span style={{ color: 'var(--muted)', whiteSpace: 'nowrap' }}> :</span>
                      <span style={{ color: c, wordBreak: 'break-word', flex: '1 1 auto' }}> {v},</span>
                    </div>
                  ))}
                  <div style={{ color: 'var(--muted)' }}>{'}'}</div>
                </div>
            ...
            {/* Skills */}
            <RevealCard index={1} preset={animPreset} noStretch>
              <div style={{ background: 'var(--black)', padding: '1.2rem', width: '100%' }}>
        ...
        <section id="CONTACT" style={{ paddingBottom: '5rem', scrollMarginTop: '60px' }}>
          <SectionHeader label="CONTACT" sub="OPEN CHANNEL" animPreset={animPreset} />
          <div style={{ display: 'flex', flexWrap: 'wrap', border: '1px solid var(--border)', background: 'var(--black)' }}>
            <RevealCard index={0} preset={animPreset} noStretch>
              <div style={{ background: 'var(--black)', padding: '1.2rem', width: '100%', borderRight: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '1rem', paddingBottom: '0.6rem', borderBottom: '1px solid var(--border)' }}>
                  ...
                  <span style={{ fontSize: '0.62rem', color: 'var(--muted)', marginLeft: '0.5rem', letterSpacing: '0.1em' }}>connect.sh</span>
                </div>
            ...
            <RevealCard index={1} preset={animPreset} noStretch>
              <div style={{ background: 'var(--black)', padding: '1.5rem', width: '100%' }}>
                <div style={{ fontSize: '0.72rem', color: 'var(--green-dim)', lineHeight: 1.9, marginBottom: '1.5rem' }}>
                  <span style={{ color: 'var(--muted)' }}>&gt; </span>
                  Ready to collaborate on apps, systems, and games. Let's build something impactful. Response time: 24 hours.
                </div>
      ...
      {/* FOOTER */}
      <footer style={{ position: 'relative', zIndex: 1, borderTop: '1px solid var(--border)', background: 'rgba(10,10,10,0.97)', fontFamily: 'var(--font-mono)' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '1.2rem 2.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
